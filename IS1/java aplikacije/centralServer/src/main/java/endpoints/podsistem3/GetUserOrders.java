...
public class GetUserOrders {
    ...
    public Response get(@PathParam("userId") String userId) {
        try (JMSContext context = connectionFactory.createContext()) {
            ...
            objMsg.setIntProperty("redniBrojZahteva", 21);
            ...
            Logger.getLogger(GetUserOrders.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity("GRESKA u GetUserOrders.get()").build();
        }
    }
}
