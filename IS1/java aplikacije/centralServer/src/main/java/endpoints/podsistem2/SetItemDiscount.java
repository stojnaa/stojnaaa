...
@Path("ps2/items/discount")
public class SetItemDiscount {
    ...
    @PUT
    public Response setDiscount(@FormParam("userId") String userId,@FormParam("itemId") String itemId,
            @FormParam("discount_pct") String discountPct) {
        try (JMSContext context = connectionFactory.createContext()) {

            HashMap<String, String> map = new HashMap<>();
            map.put("userId", userId);
            map.put("itemId", itemId);
            map.put("discount_pct", discountPct);

            ObjectMessage objMsg = context.createObjectMessage(map);
            objMsg.setIntProperty("redniBrojZahteva", 9);
            ...
            Logger.getLogger(SetItemDiscount.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity("GRESKA u SetItemDiscount.setDiscount()").build();
        }
    }
}
