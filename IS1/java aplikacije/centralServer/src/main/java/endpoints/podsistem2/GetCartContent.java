...
@Path("ps2/cart/{userId}")
public class GetCartContent {
    ...
    public Response get(@PathParam("userId") String userId) {
        try (JMSContext context = connectionFactory.createContext()) {
            ...
            objMsg.setIntProperty("redniBrojZahteva", 19);
            ...
            Logger.getLogger(GetCartContent.class.getName()).log(Level.SEVERE, null, ex);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity("GRESKA u GetCartContent.get()").build();
        }
    }
}
